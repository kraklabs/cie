...
.PHONY: all build test test-short test-coverage lint fmt fmt-check clean docker-build docker-push tools run help

# Default target
all: lint test build

build: ## Build the cie binary
	@echo "Building cie $(VERSION)..."
	@mkdir -p bin
	CGO_ENABLED=1 go build $(LDFLAGS) -o bin/cie ./cmd/cie
	@echo "✓ Built bin/cie"
