...
```
❌ cie_function(id, name, file_path, signature, code, embedding)
   Problem: Large rows, slow queries when we only need metadata
```

We split into three tables per entity:
```
✅ cie_function(id, name, file_path, signature, start_line, end_line, language)  // Lightweight ~500 bytes
✅ cie_function_code(id, code)                                                   // Lazy loaded
✅ cie_function_embedding(id, embedding)                                         // For HNSW only
```
