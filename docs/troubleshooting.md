...
cie config validate  # Validate YAML syntax

# Index health
cie status           # Shows last indexed time, function count, DB size

# Quick embedding test (if using Ollama)
curl http://localhost:11434/api/tags

# Database health
ls -lah .cie/db/     # Check database directory exists
```
...
### Issue: CozoDB Library Not Found

**Symptoms:**
- `error while loading shared libraries: libcozo_c.so: cannot open shared object file`
- `ld: library 'cozo_c' not found`
- `undefined reference to cozo_*` during build
- `cie version` fails with library loading error

**Cause:**
CozoDB C library is not installed on your system, or is installed but not in the system library path. CIE requires the CozoDB C bindings to interface with the embedded database.
...
# Verify installation
ls /opt/homebrew/lib/libcozo_c.dylib  # ARM64 Mac
ls /usr/local/lib/libcozo_c.dylib     # Intel Mac
```

**Linux (Ubuntu/Debian):**
```bash
# Download from CozoDB releases
wget https://github.com/cozodb/cozo/releases/download/v0.7.6/libcozo_c-0.7.6-x86_64-unknown-linux-gnu.so.gz
gunzip libcozo_c-0.7.6-x86_64-unknown-linux-gnu.so.gz

# Install system-wide
sudo mv libcozo_c-0.7.6-x86_64-unknown-linux-gnu.so /usr/local/lib/libcozo_c.so
sudo chmod 755 /usr/local/lib/libcozo_c.so
...
# Install
sudo mv libcozo_c-0.7.6-x86_64-unknown-linux-gnu.so /usr/lib64/libcozo_c.so
sudo chmod 755 /usr/lib64/libcozo_c.so
sudo ldconfig
```
...
```bash
cie version
# Should output: CIE version X.X.X (go1.XX.X, ...)
```

**Related:**
- [Installation Prerequisites](./getting-started.md#prerequisites)
- [CozoDB GitHub Releases](https://github.com/cozodb/cozo/releases)
...
cie status
# Should show:
# Index: ✓ (X functions, last indexed: ...)
```
...
1. **Use CIE query tools instead of raw CozoScript:**
   ...
   Instead of:
   ```bash
   # ❌ Raw CozoScript (error-prone)
   cie query "?[name] := *cie_func{name}"  # Wrong table name
   ```

   Use:
   ```bash
   # ✅ CIE query functions (validated)
   cie query --function "HandleAuth"
...
cie index
cie index --debug
rm -rf .cie/db && cie index  # Full reindex
...
| Symptom | Quick Fix |
|---------|-----------|
| Library not found | `brew install cozo` (macOS) or install libcozo_c.so (Linux) |
| No functions indexed | Check file extensions (`.go`, `.py`, `.js`, `.ts`, `.tsx`) |
| Ollama connection failed | `ollama serve` or `brew services start ollama` |
| Index corrupted | `rm -rf .cie/db && cie index` |
| Slow queries | Add `path_pattern` filter to narrow scope |
| Empty results | Lower `min_similarity` to 0.5 or try English query |
| Port conflict | `pkill cie` or change port in config |
