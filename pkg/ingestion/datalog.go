...
// formatFloatArray formats a float32 array for Datalog.
// For HNSW-compatible <F32; 1536> vectors, we must always return exactly 1536 elements.
// If input is empty or wrong size, returns a zero vector of 1536 elements.
// 1536 dimensions for Qodo-Embed-1-1.5B (768 for nomic-embed-text)
const embeddingDim = 1536

func formatFloatArray(arr []float32) string {
	// For fixed-size vector type <F32; 1536>, we need exactly 1536 elements
	if len(arr) != embeddingDim {
		// Return zero vector for empty/invalid embeddings
		zeros := make([]string, embeddingDim)
		for i := range zeros {
			zeros[i] = "0.0"
		}
		return fmt.Sprintf("[%s]", strings.Join(zeros, ", "))
	}
